...
export default function RepositoryRow({ repository, onDeleteDocument }: RepositoryRowProps) {
  ...
  const getBadgeStyles = (type?: string) => {
    switch (type) {
      ...
      case "internal":
        return "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700/50";
      case "restricted":
      ...
      default:
        return "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-800";
    }
  };
  ...
  const getDocIcon = (type: string) => {
    ...
    if (["json", "js", "ts", "md"].includes(t)) return <Icons.FileCode className="w-3.5 h-3.5 text-orange-500" />;
    return <Icons.FileArchive className="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" />;
  };
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-900 bg-white dark:bg-gray-950/20 rounded-xl overflow-hidden hover:border-gray-300 dark:hover:border-gray-800 transition-all duration-300 shadow-xs">
      ...
      >
        ...
        <div className="flex items-start space-x-3.5 min-w-0 flex-1">
          ...
          <div className="min-w-0 space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-gray-950 dark:group-hover:text-white transition-colors">
                {repository.name}
              </h4>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-500 dark:text-gray-400 font-medium leading-relaxed max-w-xl">
        ...
        {/* Stats Columns */}
        <div className="flex items-center justify-between sm:justify-end gap-6 sm:gap-8 flex-shrink-0 border-t border-gray-100 dark:border-transparent pt-3 sm:pt-0">
          {/* Docs Stat */}
          <div className="flex flex-col items-start sm:items-end">
            <span className="text-sm font-bold text-gray-800 dark:text-gray-200">
              {repository.docsCount}
            </span>
            <span className="text-[10px] text-gray-600 dark:text-gray-500 dark:text-gray-400 font-semibold tracking-wide uppercase">
              Docs
            </span>
          </div>
          ...
          <div className="relative flex items-center justify-center w-11 h-11 flex-shrink-0 select-none">
            ...
            <span className="absolute text-[9px] font-extrabold text-gray-800 dark:text-gray-200">
              {repository.progressPercent}%
            </span>
          </div>

          {/* Expand Chevron */}
          <div className="p-1 rounded-lg text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
            {isExpanded ? (
      ...
      <AnimatePresence initial={false}>
        {isExpanded && (
          ...
          >
            <div className="border-t border-gray-200 dark:border-gray-900/60 bg-gray-50/30 dark:bg-gray-950/20 p-4 space-y-3">
              <div className="flex items-center justify-between text-[10px] font-bold text-gray-600 dark:text-gray-500 dark:text-gray-400 uppercase tracking-wider pl-1">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-gray-200 dark:border-gray-900 text-[9px] font-bold text-gray-600 dark:text-gray-500 dark:text-gray-400 uppercase tracking-wider h-7">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-900/40">
                    {repository.documents.map((doc) => {
                      ...
                      return (
                        <tr key={doc.id} className="group hover:bg-gray-100/50 dark:hover:bg-gray-900/10 transition-colors h-9">
                          {/* File Icon & Name */}
                          <td className="py-2 pl-1.5">
                            <div className="flex items-center space-x-2">
                              {getDocIcon(doc.type)}
                              <span className="text-xs font-semibold text-gray-800 dark:text-gray-300 group-hover:text-gray-950 dark:group-hover:text-white transition-colors truncate max-w-[200px] sm:max-w-xs md:max-w-md">
                                {doc.name}
                              </span>
                            </div>
                          </td>
                          ...
                          {/* Size */}
                          <td className="py-2 text-xs font-medium text-gray-500 dark:text-gray-500 dark:text-gray-400 hidden md:table-cell">
                            {doc.size}
                          </td>

                          {/* Uploaded At */}
                          <td className="py-2 text-xs font-medium text-gray-500 dark:text-gray-500 hidden sm:table-cell">
                            {doc.uploadedAt}
                          </td>
                            ...
                            <div className="flex flex-col space-y-1">
                              ...
                              {isProcessing && typeof doc.progress === "number" && (
                                <div className="flex items-center space-x-1.5 w-24">
                                  ...
                                  <span className="text-[8px] font-bold text-gray-600 dark:text-gray-500 dark:text-gray-400">
                                    {doc.progress}%
                                  </span>
                                </div>
                              )}
                          ...
                          <td className="py-2 text-right pr-2">
                            <div className="flex items-center justify-end space-x-1">
                              <button
                                ...
                                className="p-1 rounded-lg text-gray-600 dark:text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors cursor-pointer"
              ...
              {repository.documents.length === 0 && (
                <div className="text-center py-6">
                  <p className="text-xs text-gray-500 dark:text-gray-400">No documents found inside this repository.</p>
                </div>
              )}
            </div>
          </motion.div>
