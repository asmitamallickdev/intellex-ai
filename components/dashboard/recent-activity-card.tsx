...
export default function RecentActivityCard() {
  ...
  const getItemStyles = (type: string, title: string) => {
    ...
    return {
      icon: FileText,
      wrapperClass: "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700/50",
    };
  };
            ...
            >
              ...
              <div className="flex items-center space-x-3.5 min-w-0 mr-4">
                ...
                <div className="min-w-0 space-y-0.5">
                  <h4 className="text-xs font-semibold text-gray-800 dark:text-gray-200 group-hover/item:text-gray-950 dark:group-hover/item:text-white transition-colors duration-150 truncate">
                    {activity.title}
                  </h4>
              ...
              <div className="flex flex-col sm:flex-row items-end sm:items-center space-y-1.5 sm:space-y-0 sm:space-x-4 flex-shrink-0">
                {/* Clock & Timestamp */}
                <div className="flex items-center space-x-1 text-[10px] text-gray-600 dark:text-gray-500 dark:text-gray-400 font-medium">
                ...
                <span className={cn(
                  ...
                  activity.status === "Indexed"
                    ...
                    : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700"
                )}>
                  {activity.status}
                </span>
                
                <ChevronRight className="hidden sm:block w-3.5 h-3.5 text-gray-400 dark:text-gray-700 group-hover/item:text-gray-600 dark:group-hover/item:text-gray-500 dark:text-gray-400 group-hover/item:translate-x-0.5 transition-all duration-200" />
              </div>
            </motion.div>
          );
        })}
